...
const tmpDir = resolve(import.meta.dirname, '../../.tmp-write-modes')
...
/**
 * The three write modes and the dry run behind them. `add` and `update` used to
 * be commands and core functions of their own, thin over exactly these two
 * modes; what they were for is what is asserted here.
 */
describe('writeTranslations', () => {
  ...
  it('upsert updates existing keys', async () => {
    // Seed the key with a low-level write, so the upsert is the only thing under test
    for (const file of localeFiles) {

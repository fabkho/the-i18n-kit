...
export function renderConfigReference(source: ConfigSource): ReferenceOutput {
  ...
  return new Map([[CONFIG_REFERENCE_PATH, page([
    ...
    `Every field the kit's configuration accepts is below, with the type it takes, what it rejects, and the description the schema itself carries. The page is generated from the zod schema in ${code(CLI_PACKAGE)} that validates your config, so it cannot list a field the tool rejects or leave out one it accepts.`,
    `This is the field list only. For where the files live and how a value reaches a running command, see [where configuration lives](/configuration/where-config-lives). For what happens when two of them declare the same key, see [precedence and conflicts](/configuration/precedence). For the values the kit reads out of your framework's own files instead of you writing them, see [declared vs. derived configuration](/configuration/declared-vs-derived).`,

...
The module has two options and a short list of situations in which its output is
not used. Both are worth knowing before you rely on it in CI.
...
Nuxt config, so a configurable producer with a fixed consumer would be a setting
whose only effect is to stop the two from meeting.

This table is written by hand. At the module's current version that is
proportionate; the [configuration field
reference](/configuration/reference), which covers the CLI's own fields, is
generated because it has to track a schema with dozens of entries.
...
fall back to loading the app through Nuxt — never as an error. A module that
cannot be trusted must not be able to break a project that worked without it.
...
When that fails, the [Nuxt adapter](/frameworks/nuxt) reports it, having first
retried with `ready: false`.

...
After this page the Nuxt module is installed, writing its artifact on every
build, and you can tell from the CLI's own output whether a command read it.
...
Module order does not matter. The module reads your i18n configuration on
Nuxt's `modules:done` hook rather than during its own setup, so it sees what
`@nuxtjs/i18n` normalized regardless of which module ran first.

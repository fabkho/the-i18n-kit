...
One command answers "what did it actually find":
...
`apps` and the project config that was loaded — followed by `layers`, one entry
per locale directory with its layer name, file count and top-level key
namespaces, alias entries marked with the layer they belong to. This is the
input to everything on the following pages, so when a report surprises you,
start here.
...
the owner's files, so `get` and `discover` still answer for the alias name.

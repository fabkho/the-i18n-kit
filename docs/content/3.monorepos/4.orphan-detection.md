...
The `orphans` and `check` commands share one line-based static scanner. What it
can and cannot see decides which keys are safe to delete.
...
`orphanScan.ignorePatterns`. The run reports; `--remove` deletes.

...
This page runs from a repository that has never seen the kit to a pipeline that
blocks a merge when translations are broken. Every step prints something you can
...
Read `detected` before you write anything. `adapter` is the one that won
detection, and `confidence` is the score it won with — not a percentage, but a
sum of signals with a different ceiling per adapter. If the adapter is wrong,
[confidence-scored detection](/frameworks/detection) explains how to force one
rather than working around a bad guess for the rest of this page.
...
a green run is not proof of a complete run. Add `--failOnFailed` to exit 2 when
any key failed.
::
...
- The `translate` gate is opt-in for a reason worth knowing before you turn it
  on: `--failOnFailed` fails a job that already committed the keys that
  succeeded.

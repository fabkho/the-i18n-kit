...
Both presets register the plugin under the `@the-i18n-kit/vue` prefix and apply to
`**/*.vue`, `**/*.ts`, `**/*.js` and `**/*.mjs`. They differ in one severity:
...
2. That exact pattern declared under `orphanScan` → `ignorePatterns` in the kit
   config — the place the scanner reads.
...
the file, since there is nowhere to declare the namespace. The pattern is accepted
from any layer's `ignorePatterns`, and the config is re-read when it changes, so a

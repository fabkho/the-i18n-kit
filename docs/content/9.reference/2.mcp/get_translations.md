...
The same operation runs from a terminal as [`the-i18n-cli get`](/reference/cli/get), whose page documents its flags. Both paths call the same `@the-i18n-kit/cli` functions, so the result is the same.
